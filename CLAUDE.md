...
- **MCP server:** `.taskflow/server/mcp_server.py` — started automatically by Claude Code via `.mcp.json`
...
Use `/start-project`, `/setup-team`, `/run-pipeline`, `/my-tasks`, or `/pipeline-status` to work the pipeline. Or invoke subagents directly:

| Subagent | Invoke when... |
|---|---|
| **taskflow-initiation-manager** | Building or ingesting a project brief (pre-pipeline) |
...
| `promote_backlog_item(backlog_id, ...)` | Promote backlog item to active feature |
